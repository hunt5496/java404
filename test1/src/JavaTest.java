...
//        Document google1 = null;
//        try {
//            google1 = Jsoup.connect("https://www.daum.net").get();
...
try {
    res = Jsoup.connect("https://www.google.co.kr").method(Connection.Method.GET).execute(); // url 안적음
    doc = res.parse();
}
catch (IOException e){
...
//System.out.println(doc.text());
        ...
        // select() 사용하기
//        Element btnK = doc.select("input[name=btnK]").first(); // cssQuery 안적음
//        String btnKvalue = btnK.attr("value"); // attributeKey 안적음
...
//        }
        ...
        System.out.println("class 값 .o3j99 로 검색 " + elements.size());
        ...
        // 전체 html문서에서 클래스 값이 L3eUgb 인 태그를 가져옴
        divList1 = element.select("div");
        // 클래스 값이 L3eUgb 인 태그의 자식 태그 중 div 태그를 모두 가져오는데
        // 클래스 값이 o3j99 인 태그를 제외함.
        ...
        // getElementByClass() 를 사용하여 지정한 Class 값이 가지고 있는 모든 태그 검색
        ...
        imgTag.attr("alt", "구글");
        attrValue = imgTag.attr("alt");
        System.out.println("attr()로 속성 값 변경 후 : " + attrValue);
