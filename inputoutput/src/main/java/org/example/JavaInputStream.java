...
public class JavaInputStream {
    ...
    public void read1() throws IOException{
        // 파일 열기
        // InputStream 가 추상 클래스이므로 객체를 생성하는 것이 불가능
        // 자식 클래스 타입의 객체(FileInputStream)를 부모 클래스인 InputStream 타입의 변수에 저장하여 사용함
        ...
        // 스트림은 자바 내부의 리소스가 아님. 외부에 존재하는 리소스이므로 사용 후 스트림을 닫지 않으면 메모리를 계속 사용하게 되어,
        // 메모리 누수 현상이 발생하게 됨.
        is.close();
    }
    ...
    public void read2() throws  IOException{
        ...
        while (true){
        // read(byte[] buff) 메소드를 사용하여 데이터를 지정한 byte 배열의 크기만큼 가져옴
